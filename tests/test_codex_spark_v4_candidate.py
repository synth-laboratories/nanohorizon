from __future__ import annotations

import json
from pathlib import Path

import yaml


REPO_ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = REPO_ROOT / "configs" / "craftax_prompt_opt_qwen35_4b_spark_v4_e2e.yaml"
SOURCE_PATH = REPO_ROOT / "src" / "nanohorizon" / "craftax_core" / "rollout.py"
RECORD_DIR = (
    REPO_ROOT
    / "records"
    / "prompt_opt_1usd_gpt54_family"
    / "2026-04-08_spark_v4_e2e"
)


def test_spark_v4_candidate_config_uses_compact_todo_scratchpad() -> None:
    payload = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    seed_prompt = str(payload["prompt"]["seed_prompt"])

    assert "compact private todo list" in seed_prompt
    assert "replace the stale target item" in seed_prompt
    assert "do not reveal the todo list" in seed_prompt.lower()


def test_rollout_source_tracks_todo_scratchpad_state() -> None:
    source = SOURCE_PATH.read_text(encoding="utf-8")
    assert "_refresh_todo_scratchpad" in source
    assert "_format_todo_scratchpad" in source
    assert "todo_scratchpad" in source


def test_spark_v4_candidate_record_bundle_is_not_run() -> None:
    metadata = json.loads((RECORD_DIR / "metadata.json").read_text(encoding="utf-8"))
    metrics = json.loads((RECORD_DIR / "metrics.json").read_text(encoding="utf-8"))
    command = (RECORD_DIR / "command.txt").read_text(encoding="utf-8").strip()

    assert metadata["implementation_status"] == "candidate_not_run"
    assert metrics["status"] == "not_run"
    assert "run_craftax_prompt_opt_qwen35_4b_gpt54_budget.sh" in command
