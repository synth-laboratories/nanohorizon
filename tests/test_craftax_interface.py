...
class CraftaxInterfaceTests(unittest.TestCase):
    def test_render_prompt_turn_uses_structured_fields(self) -> None:
        ...
        self.assertEqual(payload["decision_brief"]["mode"], "gather")
        self.assertIn("tree", payload["decision_brief"]["priority_targets"])
        self.assertIn("move toward tree", payload["decision_brief"]["primary_focus"])
    ...
    def test_decision_brief_detects_loop_risk_from_recent_history(self) -> None:
        payload = render_prompt_turn(
            "health low, energy low, no adjacent resource, night",
            history=[
                {"action": "move_right", "observation_summary": "stalled", "reward_delta": -1.0},
                {"action": "move_right", "observation_summary": "stalled", "reward_delta": -0.5},
                {"action": "move_right", "observation_summary": "stalled", "reward_delta": -0.5},
            ],
            metadata={"recent_actions": ["move_right", "move_right", "move_right"]},
        )

        self.assertEqual(payload["decision_brief"]["mode"], "stabilize")
        self.assertEqual(payload["decision_brief"]["loop_risk"], "high")
        self.assertIn("change movement pattern", payload["decision_brief"]["fallback_action"])
