...
class CraftaxInterfaceTests(unittest.TestCase):
    def test_render_prompt_turn_uses_structured_fields(self) -> None:
        ...
        self.assertEqual(payload["reward_history_summary"]["step_count"], 2)
        self.assertEqual(payload["reward_history_summary"]["net_reward"], 0.5)
        self.assertEqual(payload["reward_history_summary"]["negative_steps"], 1)
        self.assertTrue(payload["reward_history_advice"])
