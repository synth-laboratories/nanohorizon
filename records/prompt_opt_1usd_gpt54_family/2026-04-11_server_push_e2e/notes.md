...
- Strategy: `Todo Tool`
- The candidate centralizes a compact scratchpad contract in `src/nanohorizon/baselines/prompt_opt.py`.
- The runtime payload exposes `todo_item_count: 3` and `scratchpad_mode: compact-three-item`.
...
- Verification used the recorded `uv run --no-project --with pyyaml --with pytest --python 3.11 pytest -q tests/test_server_push_e2e_candidate.py`
  ...
  `PYTHONPATH=src python` smoke check of `candidate_record()` and
