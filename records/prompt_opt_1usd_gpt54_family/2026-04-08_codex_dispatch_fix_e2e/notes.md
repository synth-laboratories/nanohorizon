...
- Preserving one extra rule that keeps the full 3-4 action batch committed to the current first todo item should reduce mixed-purpose batches and repeated loops without touching the shared Craftax runtime or eval harness.

What changed:
- Added the action-dispatch rule to `src/nanohorizon/baselines/prompt_opt.py` so rollout feedback and reflection preserve it as part of the shared todo-tool contract.
- Added a prompt-opt candidate config that asks the policy to keep spending actions on the first todo item until it is satisfied, blocked, or unsafe before using the rest of the batch on lower-priority work.
