...
def _print_exports(payload: dict[str, Any]) -> None:
    print(f'export NANOHORIZON_CRAFTER_CONTAINER_URL="{payload["container_url"]}"', flush=True)
    worker_token = payload.get("container_worker_token") or ""
    print(f'export NANOHORIZON_CRAFTER_CONTAINER_WORKER_TOKEN="{worker_token}"', flush=True)


def _write_env_file(*, path: str, payload: dict[str, Any]) -> None:
    lines = [
        f'NANOHORIZON_CRAFTER_CONTAINER_URL="{payload["container_url"]}"',
        f'NANOHORIZON_CRAFTER_CONTAINER_WORKER_TOKEN="{payload.get("container_worker_token") or ""}"',
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open a public tunnel for the local NanoHorizon Crafter container")
