...
trap nanohorizon_cleanup_crafter_tunnel EXIT
nanohorizon_open_crafter_tunnel_if_needed "$ROOT"
...
if [[ "$DEPLOY_BEFORE_RUN" == "1" ]]; then
  ./scripts/deploy_crafter_offline_modal_app.sh
fi
/opt/homebrew/bin/uv run --group modal python -m nanohorizon.invoke_modal_offline \
  --config "$CONFIG_PATH" \
  ...
  --crafter-container-url "${NANOHORIZON_CRAFTER_CONTAINER_URL:-}" \
  --crafter-container-worker-token "${NANOHORIZON_CRAFTER_CONTAINER_WORKER_TOKEN:-}" \
