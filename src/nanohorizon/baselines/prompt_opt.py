...
def _build_seed_prompt_body() -> str:
    return (
        ...
        "actions unless the local state clearly supports them. Choose a short 3 or 4 action batch that "
        "follows the first todo item and, when safe, ends next to a useful target for the next turn. "
        "Think carefully, then use the `craftax_interact` tool exactly once. Return 3 or 4 valid "
        "full-Craftax actions unless the episode is already done. Use only the tool call as the final "
        "answer. Do not output JSON, prose, or a plain-text action list. As an end-to-end handoff guard, "
        "keep the scratchpad current even if the first item is already satisfied."
    )


def build_seed_prompt(candidate_label: str = "Auto Push E2E") -> str:
    del candidate_label
    return "You are a Craftax policy agent. " + _build_seed_prompt_body()
