...
app = modal.App("nanohorizon-crafter-rlvr")
image = training_image()


def _default_output_dir() -> str:
    stamp = now_utc_iso().replace(":", "").replace("+00:00", "Z")
    return f"{ARTIFACT_DIR}/rlvr/{stamp}"
...
def run(
    *,
    ...
    cmd = [
        sys.executable,
        "-m",
        "nanohorizon.baselines.rlvr",
        "--config",
        f"{REMOTE_ROOT}/{config}",
        "--output-dir",
        str(destination),
    ]
    print("Remote RLVR command:", " ".join(shlex.quote(part) for part in cmd), flush=True)
